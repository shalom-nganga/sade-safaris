...
function CarContact() {
  ...
  const inputStyle = { width: '100%', border: '2px solid #e2e8f0', borderRadius: '6px', padding: '11px 14px', fontSize: '13px', fontFamily: 'inherit', outline: 'none', transition: 'border .2s', background: '#f8fafc', boxSizing: 'border-box' };
    ...
    <>
      <div style={{ fontFamily: "'Nunito Sans', sans-serif", background: '#f0f4ff' }}>
        ...
        <section style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 60%, #1a56db 100%)', padding: '90px 5%', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2, maxWidth: '1400px', margin: '0 auto' }}>
            <p style={{ fontSize: '12px', fontWeight: 800, color: '#60a5fa', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '12px' }}>GET IN TOUCH</p>
            <h1 style={{ fontFamily: "'Oswald', sans-serif", fontSize: 'clamp(32px, 6vw, 80px)', fontWeight: 700, color: 'white', textTransform: 'uppercase', lineHeight: 1.05, marginBottom: '20px' }}>
              BOOK A VEHICLE OR<br /><span style={{ color: '#60a5fa' }}>MAKE AN ENQUIRY</span>
            </h1>
            <p style={{ fontSize: '17px', color: 'rgba(255,255,255,.75)', maxWidth: '560px', lineHeight: 1.75 }}>
              Our team is available 7 days a week to help you find the perfect vehicle, arrange a driver, or plan your journey across Kenya.
            </p>
          </div>
        </section>
        ...
        <section style={{ background: '#1a56db', padding: '36px 5%' }}>
          <div className="carcontact-strip" style={{ maxWidth: '1400px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px' }}>
            {[
        ...
        <section style={{ padding: '72px 5%' }}>
          <div className="carcontact-main" style={{ maxWidth: '1400px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 440px', gap: '48px' }}>

            {/* FORM */}
            <div>
              <p style={{ fontSize: '12px', fontWeight: 800, color: '#1a56db', letterSpacing: '2.5px', textTransform: 'uppercase', marginBottom: '10px' }}>SEND US A MESSAGE</p>
              <h2 className="carcontact-title" style={{ fontFamily: "'Oswald', sans-serif", fontSize: '38px', fontWeight: 700, textTransform: 'uppercase', marginBottom: '32px', color: '#1a1a2e' }}>
                HOW CAN WE<br /><span style={{ color: '#1a56db' }}>HELP YOU?</span>
              </h2>

              {submitted ? (
                <div style={{ background: 'white', borderRadius: '12px', padding: '60px 40px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,.07)' }}>
                  <div style={{ fontSize: '64px', marginBottom: '20px' }}>🚗</div>
                  <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '28px', fontWeight: 700, color: '#1a56db', marginBottom: '12px', textTransform: 'uppercase' }}>MESSAGE SENT!</h3>
                  ...
                  <button onClick={() => setSubmitted(false)} style={{ background: '#1a56db', color: 'white', border: 'none', padding: '13px 28px', fontSize: '13px', fontWeight: 800, cursor: 'pointer', borderRadius: '6px', fontFamily: 'inherit' }}>
                    SEND ANOTHER MESSAGE
                  </button>
                </div>
              ) : (
                <div style={{ background: 'white', borderRadius: '12px', padding: '40px', boxShadow: '0 4px 20px rgba(0,0,0,.07)' }}>
                  <div className="carcontact-row2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                    {[{ label: '👤 Full Name', name: 'name', placeholder: 'Your full name', type: 'text' }, { label: '✉️ Email', name: 'email', placeholder: 'your@email.com', type: 'email' }].map(f => (
                      <div key={f.name}>
                  ...
                  <div className="carcontact-row2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '10px', fontWeight: 800, color: '#1a56db', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: '6px' }}>📞 Phone</label>
                      <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="+254 700 000 000" style={inputStyle}
                        onFocus={e => e.target.style.borderColor = '#1a56db'} onBlur={e => e.target.style.borderColor = '#e2e8f0'} />
                    </div>
                    <div>
                  ...
                  <div style={{ marginBottom: '24px' }}>
                    ...
                    <textarea name="message" value={form.message} onChange={handleChange} placeholder="Tell us what you need — vehicle type, dates, pickup location, number of passengers..." rows={5}
                      style={{ ...inputStyle, resize: 'vertical' }}
                      onFocus={e => e.target.style.borderColor = '#1a56db'} onBlur={e => e.target.style.borderColor = '#e2e8f0'} />
                  </div>
                  <button onClick={handleSubmit} style={{ width: '100%', padding: '16px', fontSize: '14px', fontWeight: 800, background: 'linear-gradient(135deg, #1a56db, #0e3fa5)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit', letterSpacing: '.5px', boxShadow: '0 4px 16px rgba(26,86,219,.4)', transition: 'all .25s' }}
            ...
            {/* SIDEBAR */}
            <div>
              <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,.07)', marginBottom: '20px', borderTop: '4px solid #1a56db' }}>
                <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '18px', fontWeight: 700, textTransform: 'uppercase', marginBottom: '18px', color: '#1a1a2e' }}>OFFICE HOURS</h3>
                {[['Monday – Friday', '7:00 AM – 8:00 PM'], ['Saturday', '7:00 AM – 6:00 PM'], ['Sunday', 'Emergency calls only']].map(([day, hours]) => (
              ...
              <div style={{ background: 'white', borderRadius: '12px', padding: '28px', boxShadow: '0 4px 20px rgba(0,0,0,.07)' }}>
                <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '18px', fontWeight: 700, textTransform: 'uppercase', marginBottom: '18px', color: '#1a1a2e' }}>COMMON QUESTIONS</h3>
                {faqs.map((faq, i) => (
                  <div key={i} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    ...
                    {activeFaq === i && <div style={{ fontSize: '13px', color: '#64748b', lineHeight: 1.75, paddingBottom: '13px' }}>{faq.a}</div>}
                  </div>
                ))}
              </div>
            </div>
      ...
      <style>{`
        @media (max-width: 900px) {
          .carcontact-strip { grid-template-columns: repeat(2, 1fr) !important; }
          .carcontact-main { grid-template-columns: 1fr !important; }
          .carcontact-title { font-size: 28px !important; }
        }
        @media (max-width: 600px) {
          .carcontact-row2 { grid-template-columns: 1fr !important; }
          .carcontact-strip { grid-template-columns: 1fr 1fr !important; gap: 16px !important; }
        }
      `}</style>
    </>
  );
