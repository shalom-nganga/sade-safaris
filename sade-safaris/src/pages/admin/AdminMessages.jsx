...
function AdminMessages() {
  ...
  useEffect(() => { async function load() { setMessages(await getMessages()); } load(); }, []);

  const refresh = async () => setMessages(await getMessages());

  const handleSelect = async (id) => {
    setSelected(id); setReply(''); setReplySent(false);
    await markMessageRead(id); refresh();
    setShowDetail(true); // open detail on mobile
  };

  const handleReply = async () => {
    if (!reply.trim()) return;
    await replyToMessage(selected, reply);
    setReplySent(true); refresh();
  };

  const handleDelete = async (id) => {
    await deleteMessage(id);
    setSelected(null); setShowDetail(false); refresh();
  };
  ...
  return (
    <>
      <div>
        {/* Summary */}
        <div className="msg-summary" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
          {[
        ...
        {/* Panel layout */}
        <div className="msg-panels" style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '24px', height: 'calc(100vh - 280px)', minHeight: '500px' }}>

          {/* Message list — hidden on mobile when detail is open */}
          <div className={`msg-list-panel${showDetail ? ' msg-list-hidden' : ''}`} style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,.05)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '16px', borderBottom: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                {['All', 'Safari', 'Car Hire'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} style={{
                    padding: '6px 14px', fontSize: '11px', fontWeight: 700, borderRadius: '20px', cursor: 'pointer', fontFamily: 'inherit', border: '1.5px solid',
                    background: filter === f ? '#1a56db' : 'white', color: filter === f ? 'white' : '#555', borderColor: filter === f ? '#1a56db' : '#e2e8f0',
                  }}>{f}</button>
                ))}
                {unread > 0 && (
                  <span style={{ marginLeft: 'auto', background: '#ef4444', color: 'white', fontSize: '10px', fontWeight: 800, padding: '3px 8px', borderRadius: '20px' }}>{unread} new</span>
                )}
              </div>
            </div>
          ...
          {/* Detail panel */}
          <div className={`msg-detail-panel${showDetail ? ' msg-detail-visible' : ''}`} style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,.05)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            {active ? (
              <>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0' }}>
                  {/* Back button — mobile only */}
                  <button onClick={() => setShowDetail(false)} className="msg-back-btn" style={{ display: 'none', background: 'none', border: 'none', color: '#1a56db', fontWeight: 700, fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', padding: '0 0 12px 0' }}>
                    ← Back to messages
                  </button>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px', gap: '10px', flexWrap: 'wrap' }}>
                    <h3 style={{ fontFamily: "'Oswald', sans-serif", fontSize: '18px', fontWeight: 700, color: '#1a1a2e', minWidth: 0 }}>{active.subject}</h3>
                    <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                      ...
                      <button onClick={() => handleDelete(active.id)} style={{ padding: '4px 12px', fontSize: '11px', fontWeight: 700, background: '#fef2f2', color: '#ef4444', border: 'none', borderRadius: '20px', cursor: 'pointer', fontFamily: 'inherit' }}>
                        🗑️ Delete
                      </button>
                    </div>
                  </div>
                  <div className="msg-meta" style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                  ...
                  {replySent ? (
                    ...
                    <div>
                      ...
                      <textarea rows={4} value={reply} onChange={e => setReply(e.target.value)} placeholder="Type your reply..."
                        style={{ width: '100%', border: '2px solid #e2e8f0', borderRadius: '8px', padding: '12px 14px', fontSize: '13px', fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
                        onFocus={e => e.target.style.borderColor = '#1a56db'} onBlur={e => e.target.style.borderColor = '#e2e8f0'} />
                      <div style={{ marginTop: '12px' }}>
                        <button onClick={handleReply} style={{ background: '#1a56db', color: 'white', border: 'none', padding: '10px 24px', borderRadius: '6px', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                          📧 Save Reply
                        </button>
                      </div>
                    </div>
      ...
      <style>{`
        @media (max-width: 768px) {
          .msg-summary { grid-template-columns: repeat(2, 1fr) !important; }
          .msg-panels { grid-template-columns: 1fr !important; height: auto !important; }
          .msg-list-panel { min-height: 400px; }
          .msg-list-hidden { display: none !important; }
          .msg-detail-panel { display: none; min-height: 500px; }
          .msg-detail-visible { display: flex !important; }
          .msg-back-btn { display: block !important; }
        }
        @media (max-width: 480px) {
          .msg-summary { grid-template-columns: repeat(2, 1fr) !important; gap: 10px !important; }
          .msg-meta { flex-direction: column !important; gap: 6px !important; }
        }
      `}</style>
    </>
  );
